#!/usr/bin/env python3
"""Check OpenAI multimodal batch status, optionally download output/error JSONL, and print a summary."""
...
def save_file(client: OpenAI, file_id: str, target: Path) -> Path:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Retrieve and display OpenAI multimodal batch status")
    ...
    grp.add_argument(
        ...
        help=(
            ...
            "batch-openai-image-text/image-text-requests-<model>.jsonl.batch.json"
        ),
    )
    parser.add_argument(
        "--errors-out",
        help=(
            "Optional path to save the error JSONL (default: batch-openai-image-text/raw-batch-output/"
            "<basename>.errors.jsonl or errors-<batch_id>.jsonl)"
        ),
    )
    parser.add_argument(
        "--output-out",
        help=(
            "Optional path to save the output JSONL (default: batch-openai-image-text/raw-batch-output/"
            "<basename>.output.jsonl or output-<batch_id>.jsonl)"
        ),
    )
...
def main() -> None:
    ...
    if args.model and not batch_id:
