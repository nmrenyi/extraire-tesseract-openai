#!/usr/bin/env python3
"""Extract TSV results from an OpenAI multimodal batch output JSONL.
...
Example:
  python batch-openai-image-text/extract_openai_image_text_batch_output.py \
    --model gpt-5-mini-2025-08-07

Input default: batch-openai-image-text/raw-batch-output/image-text-requests-<model>.output.jsonl
Output default: batch-openai-image-text/raw-output-tsv/<model>/<custom_id>.tsv
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extract TSVs from OpenAI multimodal batch output JSONL")
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Path to output JSONL (default: batch-openai-image-text/raw-batch-output/"
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        help="Where to write TSVs (default: batch-openai-image-text/raw-output-tsv/<model>/)",
    )
...
def main() -> None:
    ...
    in_path = args.output_jsonl or (
        ROOT
        / "batch-openai-image-text"
        / "raw-batch-output"
        / f"image-text-requests-{args.model}.output.jsonl"
    )
    out_dir = args.out_dir or (
        ROOT / "batch-openai-image-text" / "raw-output-tsv" / args.model
    )
