#!/usr/bin/env python3
"""Upload a multimodal (image + OCR text) JSONL and start an OpenAI batch job.
...
Example:
  python batch-openai-image-text/run_openai_image_text_batch.py \
    --file batch-openai-image-text/image-text-requests-gpt-5-mini-2025-08-07.jsonl \
    --window 24h
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Upload JSONL and launch OpenAI multimodal batch")
...
def main() -> None:
    ...
    file_id_out = (
        Path(args.file_id_out)
        if args.file_id_out
        else jsonl_path.with_suffix(jsonl_path.suffix + ".file_id.txt")
    )
    batch_out = (
        Path(args.batch_out)
        if args.batch_out
