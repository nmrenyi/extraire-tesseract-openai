#!/usr/bin/env python3
"""Check OpenAI vision batch status, optionally download output/error JSONL, and print a summary."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Retrieve and display OpenAI vision batch status")
    grp = parser.add_mutually_exclusive_group(required=True)
    grp.add_argument("--batch-id", help="Batch ID (e.g., batch_abc123)")
    grp.add_argument(
        "--model",
        default="gpt-5-mini-2025-08-07",
        help="Model name to infer batch JSON at batch-openai-image/image-requests-<model>.jsonl.batch.json",
    )
    parser.add_argument(
        "--errors-out",
        help="Optional path to save the error JSONL (default: batch-openai-image/raw-batch-output/<basename>.errors.jsonl or errors-<batch_id>.jsonl)",
    )
    parser.add_argument(
        "--output-out",
        help="Optional path to save the output JSONL (default: batch-openai-image/raw-batch-output/<basename>.output.jsonl or output-<batch_id>.jsonl)",
    )
